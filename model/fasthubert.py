...
class FastHubertConfig(HubertConfig):
    ...
    input_feat_per_channel: int = field(
        default=80, metadata={"help": "dimension of input features of each channel"}
    )
    ...
    conv_channels: int = field(
        default=1024, metadata={"help": "number of intermediate channels"}
    )

    fbank_encoder_dim: int = field(
        default=512, metadata={"help": "number of output channels"}
    )

    conv_kernel_sizes: str = field(
        default="5,5", metadata={"help": "kernel size for each convolutional layer"}
    )

    # ILS config
    ils: bool = field(
        default=True,
        metadata={"help": "use intermediate layer supervision mechanism or not"},
    )

    predict_layers: str = field(
        default="[3,11]", metadata={"help": "intermediate layer set"}
    )

    # SpecAugment config
    freq_mask_F: int = field(
        default=30,
        metadata={"help": "maximum value of the mask width in frequency domain"},
    )

    freq_mask_N: int = field(
        default=2, metadata={"help": "number of masks in frequency domain"}
    )

    time_mask_N: int = field(
        default=2, metadata={"help": "maximum value of the mask width in time domain"}
    )

    time_mask_T: int = field(
        default=40, metadata={"help": "number of masks in time domain"}
    )

    time_mask_p: float = field(
        default=1.0, metadata={"help": "mask probability in time domain"}
    )
