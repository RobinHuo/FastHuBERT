...
from fairseq.data.audio.hubert_dataset import (
    load_label,
    load_label_offset,
...
class FastHubertDataset(HubertDataset):
    def __init__(
        ...
        if os.getenv("ONEFILE"):
            self.audio_root, self.audio_names, inds, tot, self.sizes, self.idxs = (
                load_fbank_file(
    ...
    def get_fbank(self, index):
        if self.idxs is not None:
            ...
            self.mmap.madvise(
                mmap.MADV_DONTNEED, start=ofs // mmap.PAGESIZE, length=fbank.nbytes
            )
        else:
        ...
        for i, audio in enumerate(audios):
            ...
            if diff == 0:
                ...
                collated_audios[i] = torch.cat(
                    [audio, audio.new_full((-diff, 80), 0.0)]
                )
